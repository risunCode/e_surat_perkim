...
<div class="max-w-xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-white">Edit Status</h1>
    </div>
    <form action="{{ route('admin.statuses.update', $status) }}" method="POST" class="bg-neutral-900 border border-neutral-800 rounded-xl p-6 space-y-6">
        @csrf @method('PUT')
        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1">Nama Status *</label>
            <input type="text" name="status" value="{{ old('status', $status->status) }}" required class="w-full px-4 py-2 bg-neutral-800 border border-neutral-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-white/20">
        </div>
