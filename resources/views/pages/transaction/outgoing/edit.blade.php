...
<div class="space-y-6">
    <div>
        <a href="{{ route('outgoing.show', $letter) }}" class="inline-flex items-center gap-1 mb-2 hover:opacity-70" style="color: var(--text-secondary);"><i class="bx bx-arrow-back"></i> Kembali</a>
        <h1 class="text-2xl font-bold" style="color: var(--text-primary);">Edit Surat Keluar</h1>
        <p style="color: var(--text-secondary);">{{ $letter->reference_number }}</p>
    </div>
    <form action="{{ route('outgoing.update', $letter) }}" method="POST" enctype="multipart/form-data" class="rounded-xl p-6 border" style="background-color: var(--bg-card); border-color: var(--border-color);">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="to" value="{{ old('to', $letter->to) }}" required class="w-full px-4 py-2 rounded-lg border focus:outline-none" style="background-color: var(--bg-input); border-color: var(--border-color); color: var(--text-primary);">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="file" name="attachments[]" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.gif" class="w-full px-4 py-2 rounded-lg border text-sm" style="background-color: var(--bg-input); border-color: var(--border-color); color: var(--text-primary);" onchange="previewMultipleFiles(this, 'preview-outgoing-edit')">
                <p class="text-xs mt-1" style="color: var(--text-secondary);">Maks 15MB total • Mengganti lampiran lama</p>
                <div id="preview-outgoing-edit" class="mt-2"></div>
            </div>
        ...
        <div class="flex gap-3 mt-6 pt-4 border-t" style="border-color: var(--border-color);">
            ...
            <a href="{{ route('outgoing.show', $letter) }}" class="px-4 py-2 rounded-lg" style="background-color: var(--bg-input); color: var(--text-secondary);">Batal</a>
        </div>
    </form>
</div>
@endsection
